        ...
        >
          ...
          >
            <template v-slot:activator="{ on }">
              <v-btn id="login_btn" v-on="on" text dark :ripple="false" class="icon-btn"
                     style="padding-top: 20px; text-transform: none;">
                <v-icon class="mr-2 primary--text">account_circle</v-icon>
                <h2 class="primary--text" style="text-align: center">{{ profile.username }}</h2>
                <v-icon class="primary--text">arrow_drop_down</v-icon>
              </v-btn>
            </template>
            ...
            <v-list dense>
              ...
              <v-list-item v-for="(item, i) in computedOrganisationItems" :key="i"
                           :to="{ name: 'org_projects', params: { name: item.title }}">
                {{ item.title }}
              </v-list-item>
            </v-list>
          </v-menu>
          ...
          <v-btn x-small
                 ...
                 style="float: right;"
                 id="create_org_btn"
                 @click="newOrganisationDialog">+ 创建
          </v-btn>
          <v-list :expand="true" nav>
          ...
          <span v-if="organisationItems.length === 0"
                style="font-size: 1rem; font-weight: 400; line-height: 1rem; color: #2d4470; padding-left: 18px;">暂无组织</span>
          <v-divider style="width: 90%; margin: 5px 0px 5px 0px"/>
          <span style="color: #8c8c8c; font-size: larger">
